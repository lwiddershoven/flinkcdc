...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "orders" <<-EOSQL
  ...
  GRANT USAGE ON SCHEMA orderservice TO debezium_user;

  GRANT SELECT ON ALL TABLES IN SCHEMA orderservice TO debezium_user; -- tables already present
  ALTER DEFAULT PRIVILEGES IN SCHEMA orderservice GRANT SELECT ON TABLES TO debezium_user; -- tables created in the future
